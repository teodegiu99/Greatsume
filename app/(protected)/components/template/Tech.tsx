      ...
      <div className="flex justify-between items-start mb-10">
        <div className="max-w-[70%]">
          <h1 className="text-4xl font-extrabold text-[#2d3748] tracking-tight">
            {object.name} <span className="text-blue-600">{object.surname}</span>
          </h1>
          <p className="text-xl font-medium text-gray-600 mt-2">{object.desiredJob}</p>
          <p className="text-sm text-gray-500 mt-4 leading-relaxed">{object.bio}</p>
        </div>
        
        <div className="flex flex-col items-end gap-y-2 text-sm font-medium">
          {object.email && <span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full border border-blue-100">{object.email}</span>}
          <div className="flex gap-x-3 mt-2 text-xl text-gray-700">
            {object.github && <a href={object.github} target="_blank"><FaGithub /></a>}
            {object.linkedin && <a href={object.linkedin} target="_blank"><FaLinkedin /></a>}
            {object.website && <a href={object.website} target="_blank"><FaExternalLinkAlt className="text-lg" /></a>}
          </div>
        </div>
      </div>
        ...
        <div className="col-span-1 space-y-8">
          ...
          <div className="paginate-item">
            ...
            <ul className="text-sm space-y-1 text-gray-700">
              {object.softSkillss?.map((skill, i) => (
                <li key={i}>• {skill}</li>
              ))}
            </ul>
          </div>
