...
import { MdShare, MdLink, MdLinkOff, MdContentCopy, MdCheck } from "react-icons/md";
...
const ShareSettings = () => {
  ...
  const shareUrl = publicData?.publicLink ? `${window.location.origin}/shared/${publicData.publicLink}` : "";
  ...
  const onGenerateLink = () => {
    startTransition(async () => {
      ...
      if (result?.slug) {
        setPublicData((prev: any) => ({ ...prev, publicLink: result.slug }));
      }
    });
  };
    ...
    <div className="bg-[#f8f9fa] min-h-screen pb-10">
      <div className="flex sticky top-0 bg-white/80 backdrop-blur-md justify-between items-center z-50 border-b border-slate-200 px-6 py-4 shadow-sm mb-6">
        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
          <MdShare className="text-indigo-500" />
          Share Settings
        </h2>
        <Button 
          ...
          type="submit"
          disabled={isPending}
          className="gap-x-2 bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm"
        >
          <RiSave3Fill size={18} />
          Save Settings
        </Button>
        ...
        <div className="bg-white p-6 mx-4 border border-slate-100 rounded-xl shadow-sm">
          ...
          {!publicData?.publicLink ? (
            <div className="text-center py-4">
              <p className="text-sm text-slate-500 mb-4">No public link generated yet.</p>
              <Button onClick={onGenerateLink} disabled={isPending} className="w-full bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 p-2 bg-slate-50 border border-slate-200 rounded-lg">
                <input readOnly value={shareUrl} className="bg-transparent border-none outline-none text-sm text-slate-600 flex-1 px-2" />
                <Button onClick={handleCopy} variant="ghost" size="sm" className={copied ? "text-green-600" : "text-indigo-600"}>
                  {copied ? <MdCheck size={18} /> : <MdContentCopy size={18} />}
                </Button>
              </div>
              <Button onClick={onDeleteLink} disabled={isPending} variant="destructive" className="w-full gap-2 text-xs opacity-70 hover:opacity-100">
                <MdLinkOff size={16} />
                Disable & Delete Link
              </Button>
            </div>
